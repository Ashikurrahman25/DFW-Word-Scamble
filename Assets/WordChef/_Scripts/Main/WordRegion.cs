...
public class WordRegion : MonoBehaviour {
    ...
    public List<LineWord> lines = new List<LineWord>();
    private List<string> validWords = new List<string>();

    private GameLevel gameLevel;
    public int numWords, numCol, numRow;
    ...
    public void HintClick()
    {
        ...
        {
            MainController.instance.currentHint = 0;
            DialogController.instance.ShowDialog(DialogType.Quiz);
        }
    ...
    public void ShowHint(int currentHint)
    {
        //var line = lines.Find(x => !x.isShown);
        ...
        for (int i = 0; i < Math.Ceiling(((double)lines[currentHint].cells.Count * 50)/100); i++)
        {
            lines[currentHint].ShowHint();
        }

        if(numCol > 1)
        {
            if((currentHint + numRow) <= lines.Count-1)
            {
                Debug.Log((lines[currentHint + numRow].cells.Count * 50) / 100);

                for (int i = 0; i < Math.Ceiling((double)lines[currentHint + numRow].cells.Count * 50)/100; i++)
                {
                    lines[currentHint + numRow].ShowHint();
                }
            }
        }
